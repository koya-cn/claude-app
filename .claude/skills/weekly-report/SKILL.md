...
**除外** — 実作業でないノイズは落とす:
- メタ操作: `/model` `/mcp` `/observer-start` `/login` `/clear` 等で、**実作業を伴わない**もの。
- 中身のない 1 prompt のみの瑣末セッション（`📝 1 prompt`）。

⚠️ **タイトルだけで除外しない（重要）。** 一覧のタイトルは「冒頭発話」か `/resume`・`/clear` 等の**コマンド名**にすぎず、ターン抜粋も末尾中心のため、作業内容が見えないことがある（例: タイトル `/resume`・末尾が commit/push/PR だけのセッションが、実は AWS 設定などの実作業だった）。除外候補が次のいずれかに当てはまる場合は、**セッション実体(jsonl)の冒頭ユーザー発話を確認してから**含める／外すを判断する:
- タイトルが `/resume` 等のコマンド名で、テーマが読み取れない
- prompt 数が多い（目安 `📝 5 prompts` 以上）— 再開された実作業のことが多い
- ターン抜粋から実施内容が特定できない

冒頭発話の確認方法（resume コマンド行の `<session-id>` を使う）:
```bash
f=$(find ~/.claude /mnt/c/Users/*/.claude -name '<session-id>.jsonl' 2>/dev/null | head -1)
# その jsonl の type=="user" 行から、コマンド/caveat 以外の冒頭発話を数件抽出してテーマを把握する
```
`/resume` でも prompt 数が多ければ実作業の再開。一律には落とさない。
...
- 未完・継続作業は「今週の予定」へ（優先度付き）。

⚠️ **見出しの時制に注意。** このコマンドは対象週の**翌週**（月曜の報告会前など）に実行される想定。したがって対象週の実績は読者から見て「先週」、未完・継続作業の予定は「今週」になる。「今週の概要」「来週の予定」とは書かない。
...
- [ ] 除外候補をタイトル/末尾抜粋だけで判断していない（`/resume` 等・prompt 数が多い・テーマ不明は jsonl の冒頭発話を確認した）
- [ ] 各プロジェクトにステータスがある
- [ ] 未完項目が「今週の予定」に優先度付きで入っている
- [ ] 見出しの時制が実行タイミング基準になっている（実績=「先週やったこと」、予定=「今週の予定」）
